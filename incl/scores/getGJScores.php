...
if($type == "top" OR $type == "creators" OR $type == "relative"){
	if($type == "top"){
		$query = "SELECT * FROM users WHERE isBanned = '0' AND gameVersion $sign AND stars > 0 ORDER BY stars DESC LIMIT 50";
	}
	if($type == "creators"){
		$query = "SELECT * FROM users WHERE isCreatorBanned = '0' ORDER BY creatorPoints DESC LIMIT 50";
	}
	if($type == "relative"){
